...
<div class="row" id="content-wrap">
  <div class="twelve columns" id="content">
        ...
        <div class="contact-items-wrap" id="block-system-main">
               ...
               <h4 class="lead">RoomType Information</h4>
              
                  <div>
                    <label>{{Lang::get('admin.roomtype_image')}}</label>
                    <div><img src="{{ URL::to('/').str_replace('server/php/files/','server/php/files/thumbnail/',$roomtypes['roomtype_image']) }}"/></div>
                    <div class="btn btn-success fileinput-button">
                    <i class="glyphicon glyphicon-plus"></i>
                    <span class="yellow_btn">update Photo...</span>
                ...
                <form action="{{ URL::to('/backoffice/settingroomtypeupdate')}}" method="post" accept-charset="UTF-8" id="roomstype_add" enctype="multipart/form-data">
                  <div>
                    <label>{{Lang::get('admin.roomtype_name')}}</label>
                    <input type="text" name="roomtype_name" id="roomtype_name" class="form-text" value="{{$roomtypes['roomtype_name']}}" />
                    <input id="roomtype_image" type="hidden" name="roomtype_image">
                  </div>
                  <div>
                    <label>{{Lang::get('admin.roomtype_price')}}</label>
                    <input <?php if(isset($update_status)){ echo "readonly=readonly"; } ?> type="text" name="roomtype_price" id="roomtype_price" class="form-text" value="{{$roomtypes['roomtype_price']}}" />
                  </div>
                  <div>
                    <label>{{Lang::get('admin.roomtype_maxperson')}}</label>
                    <input <?php if(isset($update_status)){ echo "readonly=readonly"; } ?> type="number" name="roomtype_maxperson" id="roomtype_maxperson" class="form-text"value="{{$roomtypes['roomtype_maxperson']}}" />
                  </div>
                  <div>
                    <label>{{Lang::get('admin.roomtype_maxbedsupport')}}</label>
                    <input <?php if(isset($update_status)){ echo "readonly=readonly"; } ?> type="number" name="roomtype_maxbedsupport" id="roomtype_maxbedsupport" class="form-text" value="{{$roomtypes['roomtype_maxbedsupport']}}" />
                    <input type="hidden" name="roomtype_id" id="roomtype_id" class="form-text" value="{{$roomtypes['id']}}" />
                  </div>
                  <div>
                    <label>{{Lang::get('admin.include_breakfast')}}</label>
                    <input <?php if($roomtypes['include_breakfast']==true){ echo "checked"; }?> name="include_breakfast" type="checkbox" value="1">
                  </div>
                    @if(isset($update_status))
                      @if($update_status==true)
                        <div class="info" >Information updated...</div>
                      @else
                        <div class="warning" >Information update failed please contact admin...</div>
                      @endif
                    @else
                    <div class="form-actions form-wrapper" id="edit-actions">
                        <input type="button" id="save" value="Save" class="form-submit yellow_btn" />
                    </div>
                    @endif
              </form>
        </div>
